...
"""
        ...
        issues = analyzer.analyze(Path("test.py"), code)
        assert len(issues) > 0
        assert any("circular" in issue.message.lower() or "inside function" in issue.message.lower() 
                  for issue in issues)
...
"""
        ...
        issues = analyzer.analyze(Path("test.py"), code)
        assert len(issues) >= 2
        assert any("always" in issue.message.lower() or "redundant" in issue.message.lower() 
                  for issue in issues)
...
"""
        ...
        issues = analyzer.analyze(Path("test.py"), code)
        assert len(issues) >= 2
        assert any("incomplete" in issue.message.lower() or "generic" in issue.message.lower() 
                  for issue in issues)
